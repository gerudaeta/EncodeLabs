...
- Checks pending: image-specific ngrok `/api/endpoints`/policy and LAN-denial validation with explicit authorization and valid credentials; optional live Telegram smoke; remote delivery authorization. This local implementation is complete, but the five-service public behavior is NOT verified.

## Next step

Await explicit authorization for the destination, operation, and credential/session before any selected-image pull or public-tunnel/Telegram test. Only after live public-policy and LAN-denial checks pass may the five-service path be described as verified. Do not push or open a PR without separate remote-delivery authorization.
